...
### `interactions`
Cache table for interaction outcomes. Looked up by `item_id` + `object_id` + `required_state`.

| Column | Type | Notes |
|--------|------|-------|
| id | uuid | PK, auto |
| item_id | uuid | FK → `items(id)`. Nullable (`null` = bare hands) |
| object_id | uuid | FK → `objects(id)`. NOT NULL |
| required_state | text | Object state required. Nullable (`null` = any state) |
| item_tags | text[] | Context: item tags at time of interaction |
| object_tags | text[] | Context: object tags at time of interaction |
...
| source | text | `'manual'` (admin-created) or `'ai'` (generated by Gemini) |
| created_at | timestamptz | Auto |

**Lookup:** `WHERE item_id = ? AND object_id = ? AND (required_state IS NULL OR required_state = ?)`. Exact state match wins over wildcard.

**Cache misses** go to Gemini AI. The result is cached with the specific `item_id`/`object_id` so repeated use is instant.

**Admin panel** lets you seed specific combos (e.g., Coffee Cup + Coffee Machine → Filled Coffee Cup).
